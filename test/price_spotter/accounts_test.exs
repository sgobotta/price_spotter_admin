defmodule PriceSpotter.AccountsTest do
  ...
  describe "apply_user_email/3" do
    ...
    test "validates maximum value for email for security", %{user: user} do
      ...
      assert dgettext("errors", "must have the @ sign and no spaces") in errors_on(
               changeset
             ).email
    end
    ...
    test "validates current password", %{user: user} do
      ...
      assert %{current_password: [dgettext("errors", "is not valid")]} ==
               errors_on(changeset)
    end
  ...
  describe "generate_user_session_token/1" do
    ...
    test "generates a token", %{user: user} do
      {token, _tokens} = Accounts.generate_user_session_token(user)
